...
open class HubCloud : ExtractorApi() {
    ...
    override suspend fun getUrl(
        ...
        val rawSize = document.selectFirst("i#size")?.text().orEmpty()
        val size = rawSize.substringBefore("|").replace(Regex("(?i)views.*"), "").trim()
        ...
        document.select("a.btn").amap { element ->
            ...
            if (blocked.any { it in link }) return@amap
            ...
            when {
                ...
                "10gbps" in label -> {
                    val resp = app.get("$link/download", referer = link, allowRedirects = false)
                    val dlink = resp.headers["location"]
                        ?: resp.headers["Location"].orEmpty()
                ...
                "mega server" in label -> {
                    ...
                    loadExtractor(link, realUrl, subtitleCallback, callback)
                }
            }
        }
    }
...
class GDFlix : ExtractorApi() {
    ...
    override suspend fun getUrl(
        ...
        val newUrl = try { 
            app.get(url.replace(mainUrl, latestUrl)).document.selectFirst("meta[http-equiv=refresh]")?.attr("content")
                ?.substringAfter("url=")
        } catch (e: Exception) {
            Log.e("GDFlix", "Redirect error: ${e.localizedMessage}")
            null
        } ?: url.replace(mainUrl, latestUrl)

        val document = app.get(newUrl).document
        val fileName = document.select("ul > li.list-group-item:contains(Name)").text()
            .substringAfter("Name : ")
        val rawFileSize = document.select("ul > li.list-group-item:contains(Size)").text()
            .substringAfter("Size : ")
        val fileSize = rawFileSize.substringBefore("|").replace(Regex("(?i)views.*"), "").trim()
